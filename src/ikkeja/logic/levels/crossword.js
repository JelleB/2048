...
/**
 * Picks the warehouse row Meike may flash: exact location match, or a single search hit.
 * @param {string} sessionSeed
 * @param {DutchCrosswordPuzzle} puzzle
 * @param {string} query
 * @returns {WarehouseClueEntry|null}
 */
export function resolveWarehouseFlashEntry(sessionSeed, puzzle, query) {
  const normalizedQuery = normalizeWarehouseLocation(query);
  if (!normalizedQuery) {
    return null;
  }

  const items = filterWarehouseLocations(sessionSeed, puzzle, query);
  const exact = items.find(
    (entry) => normalizeWarehouseLocation(entry.location) === normalizedQuery,
  );
  if (exact) {
    return exact;
  }
