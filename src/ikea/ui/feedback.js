...
export function wobbleElement(el) {
  el.classList.remove('ikea-wobble');
  void el.offsetWidth;
  el.classList.add('ikea-wobble');
  window.setTimeout(() => el.classList.remove('ikea-wobble'), 600);
}
...
export function glowSuccess(el) {
  el.classList.add('ikea-success-glow');
  window.setTimeout(() => el.classList.remove('ikea-success-glow'), 1500);
}
