...
JavaScript browser games: **2048**, **2248**, **Knoppenspel**, **ToneGrid** (step sequencer), **BSharp** (perfect-pitch trainer), and **Lost in IKKE-JA** (co-op escape room). Phaser 3 renders and handles input for the tile games; **game rules live in `src/logic/`** and are covered by Vitest. ToneGrid audio uses **Tone.js** in `src/audio/`. BSharp is vendored at `vendor/bsharp/` (git subtree) and runs as a standalone HTML page. IKKE-JA escape room is vanilla JS in `src/ikkeja/`.
...
| `src/navigation.js` | Menu ↔ standalone pages (BSharp, IKKE-JA) |
| `src/bsharp/entry.ts` | Vite entry: loads vendor BSharp TS + SCSS |
| `src/ikkeja/` | Lost in IKKE-JA escape room (logic, UI, audio) |
| `ikke-ja.source.html` | IKKE-JA escape room HTML shell (Vite entry) |
...
6. Menu → Lost in IKKE-JA: P1 generates session code; P2/P3 join; all four levels + victory certificate work on three roles.
